...
The exact-release OKX browser journey also reached `FINALIZED + SUCCESS` and read back `ASSESSED`, `CONSISTENT` on the live application. The wallet integration uses standard injected-provider requests and does not depend on wallet Snap APIs.
